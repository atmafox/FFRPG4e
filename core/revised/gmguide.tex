...
The antagonists should have at least a basic attack, which can be physical or magical. This is its \taction{Attack} action. Choose an Offensive Stat and a Defensive Stat for the attack and its damage. For example, an attack of an antagonist between level 10 and level 18 should cause between 3x and 7x Stat level damage.
...
\subsection{Antagonists and Critical Hits}\label{gm-criticals}
...
If you’re new to this system, be wary that a lucky critical with double damage can one-shot a low Earth mage or Freelancer at early levels. Should your group feel this is unfair and could detract from the experience at lower levels (before level 15), you could use the 150\% critical hits option or remove them entirely from your antagonists. 
Critical hits can, however, throw a wrench into the player’s battle tactics and can be exploited to increase the tension, and will be an important tool in your arsenal to design interesting antagonists and battles.
...
\textit{Heat Wave}: All combatants receive \tstatus{Fire Vulnerable} and \tstatus{Ice Resist} statuses this battle. This has no effect on combatants that already have \tstatus{Vulnerable}, \tstatus{Resist}, \tstatus{Immune} or \tstatus{Absorb} Fire (for the \tstatus{Fire Vulnerable}) or Ice (for the \tstatus{Ice Resist}). 

%\textit{Strong Winds (X)}: Increase the difficulty of all Ranged attacks by 30 this battle. Combatants with \tstatus{Flight} take X Air-elemental physical damage each time they take an action or reaction. Any combatant may use an action to land and lose the \tstatus{Flight}. 

\textit{Deep Water}: All combatants lose the \tstatus{Float}, \tstatus{Strenghten: Speed} and \tstatus{Haste} statuses and receive \tstatus{Slow} and \tstatus{Weaken: Speed} statuses this battle. This has no effect on combatants that have \tstatus{Flight} or are \textbf{Aquan}. 

\textit{Thunderstorm (X)}: At the end of every round, one random combatant is struck for X physical Lighnting-elemental damage.

\textit{Healing Circle}: All healing effects are doubled. Each time a healing effect is used, the user receives the \tstatus{Zombie} effect until the end of next round.

\textit{Rooftops}: Some combatants are sitting on rooftops. Combatants on rooftops cannot target or be targeted by combatants on the lower floor with melee attacks. It takes two actions to move between the rooftops and the lower floor. Whenever a combatant is struck while climbing (after it spends the first action to climb up or down but before it spends the second action), it takes double damage. \tstatus{Flying} characters may move freely between the lower ground and the rooftop.

\textit{Quicksand (X)}: At the start of each round, each combatant receives the \tstatus{Condemn} status effect for the next (X) rounds. This effect automatically fails on targets under the \tstatus{Float}, \tstatus{Flight} or \tstatus{Condemn} status effects. When a character receives the \tstatus{Float} or \tstatus{Flight} status effects, it loses the \tstatus{Condemn} status granted by the quicksand. Any combatant may use the \taction{Flee} action to lose the \tstatus{Condemn} status.
