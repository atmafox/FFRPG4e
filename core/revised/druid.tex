...
\begin{tabjob}
    \multicolumn{2}{p{\textwidth}}{\tability{Nature's Path}: Core ability acquired at leavel 1. You are a druid, and gain the multipliers and equipment choices above. In addition, choose one of the options below:} \\
    \crystal{level}{12pt} & %
    ...
    \crystal{level}{12pt} & %
    \textit{Geomancer}: One status effect is permanently applied to you. Which status is gained is based on your lowest Stat value when you reach level 30. You may choose between status in case of a tie: \telem{Earth} --- \telem{Puncture} \tstatus{Resist}; \telem{Air} --- \tstatus{Float}; \telem{Fire} --- \tstatus{Premonition}; \telem{Water} --- \tstatus{Blink}. \\ % chktex 8 it doesn't like any dashes
    \crystal{level}{12pt} & %
