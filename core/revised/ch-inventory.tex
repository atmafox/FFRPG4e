...
Regardless of how the Rogue earn this extra Gil, the group shall not be punished for having a Rogue. Rogues with \tability{Gilionaire} should earn on average 12.5 Gil per experience point, and Rogues with the other Abilities could earn even more, depending on their rolls. The Game Master must resist the temptation to give smaller rewards to its players because “The Rogue will steal a difference.”
\end{multicols}
\clearpage
...
\begin{multicols}{2}
Each character can equip up to two accessories at the same time.  However, the equipment's magic interferes with each other.  Thus, if two accessories or an accessory and another piece of equipment including a weapon or armor would have the same effect, they do not stack and only the most powerful item of this type will affect you.  Similarly if a character equips three or more accessories, their effects cancel each other and the character will not receive the benefits of any of them.  For example, if a character equips the Wyglar armor (+ 20\% Max HP) and the Orrachea Armlet (+ 10\% Max HP), the HP increase is only 20\%.
...
\subsubsection{General Accessories}\label{subsec:inv-gacc}

\begin{multicols}{2}
These accessories are grouped into general types that should be further customized before bought or found. Accessories whose effect mention a Stat Value or Stat Level must refer to a specific Stat (be it Earth, Air, Fire or Water). Accessories whose effect mention a specific Element must refer to a specific Element (other than Crush, Cut and Puncture). Unlike other accessories, a Rogue may swap the Stat or Element using \textit{Invention} as if they were weapons or armors. Feel free to change the names of the acessories to further reflect their nature: an \textit{Elemental Bracelet} that confers Immunity to Ice may be named \textit{Icicle Bracelet}, for instance.

\end{multicols}

\begin{tabacc}[label=inv-acc1]
    \input revised/generated/acc-boosters
\end{tabacc}

\subsubsection{Special Accessories}\label{subsec:inv-spacc}

\begin{multicols}{2}
Unlike the General Acessories, these items cannot be further customized and take up a much diverse role. They also cannot be influenced by \textit{Invention}, \textit{Blacksmith} or other equipment-modifying abilities. 
\end{multicols}

\begin{tabacc}[label=inv-acc2]
...
Some groups do not like the shared Inventory rule. In that case, you may play with each character having its own individual items. If you use this rule, only character who has the item on its own inventory may use \taction{Item} to use the consumable or equip the weapon. \pw%
